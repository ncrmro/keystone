...
8. **Archive report to zk notes**
   - The fleet_survey.md and validation_report.md contain personal deployment data — they must be archived to the user's zk notebook, NOT left only in the keystone open-source repo's `.deepwork/` folder
   - Create a dated note in `~/notes/` summarizing the run:
     ```bash
     zk new --notebook ~/notes --title "keystone fleet health $(date +%Y-%m-%d)" --no-input
     ```
   - Write a concise summary to the created note file with:
     - Keystone revision gap (commits behind, any impactful changes)
     - Host reachability table
     - Failed units per host
     - Issues found with GitHub links
     - Action items
   - The note filename will be auto-generated by zk (format: `YYYYMMDDHHMM keystone-fleet-health-YYYY-MM-DD.md`)
