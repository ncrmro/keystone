        ...
        in
        {
          default = pkgs.mkShell {
            ...
            packages = with pkgs; [
              ...
              # Keystone agent for sandbox management
              (pkgs.callPackage ./packages/keystone-agent { })
            ];
            ...
            shellHook = ''
              ...
              echo ""
              echo "Agent sandbox commands:"
              echo "  keystone-agent start   - Start a sandbox"
              echo "  keystone-agent ssh     - SSH into sandbox (with token forwarding)"
              echo "  keystone-agent tokens  - Show available API tokens"
              echo ""
              echo "Rust packages:  packages/keystone-ha/"
              echo "Node packages:  packages/keystone-installer-ui/"

              # Keystone Agent aliases for convenience
              alias agent='keystone-agent'
              alias sa='keystone-agent'
              alias sa-start='keystone-agent start'
              alias sa-ssh='keystone-agent ssh'
              alias sa-sync='keystone-agent sync'
              alias sa-stop='keystone-agent stop'
            '';
