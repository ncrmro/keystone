...
**Working Directory:** ${_AGENTCTL_PROJECT_PATH}"
    ...
    exec sudo -u "agent-${AGENT_NAME}" "$HELPER" exec bash -c '
      ...
      # Compose system prompt from conventions + AGENTS.md + role + project context
      # (applies to all AI tools; injection mechanism is per-tool below)
      # Loading order (REQ-017.6):
      #   1. System-wide conventions (~/.config/keystone/AGENTS.md)
      ...
      # 1. System-wide conventions from keystone (generated by home-manager)
      SYSTEM_AGENTS_MD="$HOME/.config/keystone/AGENTS.md"
      if [ -f "$SYSTEM_AGENTS_MD" ]; then
        SP="$(cat "$SYSTEM_AGENTS_MD")"
      fi
