...
case "$CMD" in
  ...
  email)
    ...
    ;;
  shell)
    exec sudo -u "agent-${AGENT_NAME}" "$HELPER" exec bash -c "cd /home/agent-${AGENT_NAME} && exec bash -l"
    ;;
...
**Working Directory:** ${_AGENTCTL_PROJECT_PATH}"
    ...
    exec sudo -u "agent-${AGENT_NAME}" "$HELPER" exec bash -c '
      ...
      #   2. Home identity docs (SOUL.md, TEAM.md, SERVICES.md)
      #   3. Project AGENTS.md (project-specific context)
      #   4. Role composition via --role/--roles (appended)
      SP=""

      # 2. Agent identity from the agent home. These files are symlinked from
      # ks-config/agents by home-manager activation.
      for identity_file in "$HOME/SOUL.md" "$HOME/TEAM.md" "$HOME/SERVICES.md"; do
