...
# Canonical persona file (keystone.os.agents.<name>.persona). Empty when null.
AGENT_PERSONA_FILE=""
set_agent_persona "$AGENT_NAME"
...
**Working Directory:** ${_AGENTCTL_PROJECT_PATH}"
    ...
    exec sudo -u "agent-${AGENT_NAME}" "$HELPER" exec bash -c '
      ...
      # Compose system prompt from project + role layers; persona is delivered
      # via each tool'\''s native subagent mechanism, not via SP composition.
      # Layers:
      ...
      #   2. Agent persona — sourced from keystone.os.agents.<name>.persona,
      #      materialized at ~/.claude/agents/<name>.md by the
      #      keystone.terminal.agents module. claude loads it via
      #      `--agent "$AGENT_NAME"`; codex prepends file contents to $SP.
...
$ROLE_PROMPT"
      ...
      # Layer 2 (persona) flows differently per tool: claude uses --agent so
      # Claude Code loads ~/.claude/agents/<name>.md natively; codex has no
      # native subagent dir, so we prepend the persona file contents to $SP
      # before writing the model_instructions_file.
      AGENT_PERSONA_FILE="'"$AGENT_PERSONA_FILE"'"
      AGENT_NAME_INNER="'"$AGENT_NAME"'"
      SP_FLAGS=()
      CODEX_MODEL_INSTRUCTIONS_FILE=""
      case "'"$CMD"'" in
        claude)
          if [ -n "$AGENT_PERSONA_FILE" ] && [ -f "$HOME/.claude/agents/$AGENT_NAME_INNER.md" ]; then
            SP_FLAGS+=("--agent" "$AGENT_NAME_INNER")
          fi
          if [ -n "$SP" ]; then
        ...
        codex)
          ...
          # prompt (persona + project + role) through its supported
          # model_instructions_file config key.
          if [ -n "$AGENT_PERSONA_FILE" ] && [ -f "$AGENT_PERSONA_FILE" ]; then
            if [ -n "$SP" ]; then
              SP="$(cat "$AGENT_PERSONA_FILE")
