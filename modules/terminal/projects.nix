...
# REQ-010.7  sessionPrefix defaults to "obs" for Zellij session names
{
  config,
  lib,
  ...
...
in
{
  ...
  options.keystone.projects = {
    ...
    sessionPrefix = mkOption {
      type = types.str;
      default = "obs";
      description = "Prefix for Zellij session names created by `pz` (e.g., '{prefix}-{slug}').";
      example = "work";
    };
  };
