...
NamePretty = "Install"
Description = "Package install actions"
...
Action = command_path("keystone-package-menu") .. " dispatch '%VALUE%'"

function GetEntries()
    local handle = io.popen(command_path("keystone-package-menu") .. " entries-json 2>/dev/null")
    if not handle then
        return {}
    end
