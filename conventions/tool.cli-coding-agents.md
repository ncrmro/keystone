...
<consumer-flake>/agents/
  _shared/                            committed; canonical for all tools
...
symlink to `_shared/AGENTS.md`. SKILL.md, subagent files, and the rendered
instruction file at the resolved `_shared/` path are regular files — each
tool reads them normally.

The committed surface in a consumer flake is just `_shared/`: one
`AGENTS.md` (and optionally one `skills.yaml`). Everything under
`claude/`, `gemini/`, and `codex/` is regenerated by `ks sync-agent-assets`
and gitignored — see the recommended `.gitignore` entries below.
...
is the single committed source of truth for the instruction file and the
skill schema. Everything under `agents/{claude,gemini,codex}/` is rendered
output, gitignored. First-time setup on a fresh host or upgrade from the
pre-symlink era:

```
ks sync-agent-assets       # writes _shared/AGENTS.md and renders the per-tool dirs
cd <consumer-flake>
git status                 # only _shared/AGENTS.md should show as untracked
...
git add agents/_shared/AGENTS.md .gitignore
