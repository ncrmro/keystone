...
Notes that track agent tasks are permanent notes in `notes/` with a namespaced tag set. These tags
are consumed by the `notes/task` DeepWork workflow for creation, enrichment, and auditing.
...
31. Every task note MUST include at least one `project/<slug>` tag at creation time.
32. Every task note MUST include exactly one `status/<state>` tag at all times.
33. The initial `status` tag MUST be `status/in-progress` when the note is created.
34. Artifact tags (`milestone:`, `issue:`, `pull_request:`) SHOULD be added as soon as the artifact exists.
35. The `repo:` tag SHOULD be added when the task involves a specific repository.
36. Tags MUST be lowercase and hyphenated.
37. When a task status changes, the old `status/*` tag MUST be replaced — only one `status/*` tag at a time.
38. If a task spans multiple repositories, multiple `repo:` tags MAY be present.
39. If a task has no associated milestone, the `milestone:` tag MUST be omitted.
40. Tag enrichment on archived notes SHOULD be avoided — archived notes are frozen state.
...
A **shared surface** is a VCS artifact (issue, PR, milestone) visible to all collaborators on a
platform. Shared surfaces are the canonical coordination record for any task involving more than
one agent or human. The `notes/task` workflow includes a `comment_shared_surface` step that posts
progress updates to these artifacts so any participant can resume context without reading the full
task note.
