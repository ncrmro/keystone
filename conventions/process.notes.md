...
17. Slidev decks and other Markdown-native presentation artifacts MUST be
    stored as `presentation` notes in `docs/presentations/`.
18. Presentation notes MUST include these frontmatter fields: `id`, `title`,
    `type`, `created`, `author`, `tags`, and `presentation_kind`. `project` MAY
    be omitted for non-initiative decks.
19. Initiative presentation decks MUST link to the relevant hub note, and the
    hub SHOULD link back to the active deck.
20. Presentation notes SHOULD include a concise statement of objective,
    references to source notes or decisions, and explicit next actions when the
    deck drives follow-up work.
21. Presentation notes SHOULD preserve valid Slidev frontmatter, slide
    separators, and any speaker-note links needed to trace source material.

## Decisions and VCS continuity

22. Agents MUST record repo-level or initiative-level decisions that materially affect
    implementation, operations, or prioritization in the zk notebook.
23. If a decision relates to a Git issue or pull request, the note MUST link to
    the tracker item, and the tracker item MUST reference the note ID or path.
24. Decision notes SHOULD live in `decisions/` when the decision is durable and
    worth preserving independently of a single report.
25. If a report captures a short-lived operational decision, the report MAY hold
    it directly, and a relevant hub SHOULD link to that report when one exists.

## Tagging

26. Tags MUST use a constrained namespace.
27. The primary tags are:
    - `project/<slug>`
    - `repo/<owner>/<repo>`
    - `report/<kind>`
    - `presentation/<kind>`
...
28. Project tags MUST be the primary discovery path for initiative-scoped notes.
29. Repo tags SHOULD be added when a note materially concerns one specific repo and MAY be the primary discovery path for operational reports or decks.
30. Agents SHOULD NOT introduce new tag namespaces.
31. Agents MAY introduce new values within an approved namespace only when the
...
32. If a workflow appears to need an ad hoc tag outside the approved namespaces,
    the agent SHOULD avoid creating it and SHOULD prefer frontmatter fields or
    explicit links instead.

## Archival lifecycle

33. When an initiative is completed, abandoned, or superseded, its active hub note
    and associated initiative-specific notes SHOULD be moved to `archive/`.
34. Archived notes MUST replace `status/active` with `status/archived`.
35. Archived notes SHOULD record `archived_at`, `archived_reason`, and
    `archived_from` in frontmatter when the workflow performs the move.
36. Archiving MUST preserve backlinks, report chains, presentation deck
    provenance, and project discoverability.
37. Active workflows, dashboards, and periodic reviews SHOULD exclude
    archived notes by policy unless the task is historical research.

## Agent workflow integration

38. Before starting work, agents SHOULD search for the relevant hub and recent
    reports with tags that match the note's identity, such as `project/<slug>`,
    `repo/<owner>/<repo>`, `report/<kind>`, or `presentation/<kind>`.
39. After completing a task that produced meaningful findings, agents SHOULD
    create or update either a project decision note, a report note, or a
    presentation note.
40. DeepWork workflows that produce documentation for later use MUST write their
    durable output into the notebook rather than leaving it only in scratch files.
41. Inbox processing workflows SHOULD attach promoted notes to a hub when the
