...
fn test_generated_workstation_config_evaluates() {
    eval_generated_config("eval-workstation", MachineType::Workstation, StorageType::Zfs);
}
...
fn test_generated_workstation_config_builds() {
    build_generated_config("build-workstation", MachineType::Workstation, StorageType::Zfs);
}
...
fn test_keystone_iso_builds() {
    ...
    let flake_ref = format!(
        "path:{}#packages.x86_64-linux.iso",
        root.display(),
    );

    let output = Command::new("nix")
        .args(["build", &flake_ref, "--out-link", &iso_link.display().to_string()])
        .current_dir(&root)
        .output()
    ...
    assert!(
        iso_dir.exists(),
        "ISO build output does not exist at {}", iso_dir.display(),
    );
...
/// The crate is at packages/keystone-tui/, so the repo root is two levels up.
...
fn make_config(hostname: &str, machine_type: MachineType, storage_type: StorageType) -> GenerateConfig {
    GenerateConfig {
        ...
        user: UserConfig {
            ...
            authorized_keys: vec![
                "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAItest test@host".to_string(),
            ],
        },
...
fn write_generated_config(
    ...
    std::fs::write(dir.path().join("flake.nix"), &flake_nix)
