...
/// Copy the prepared installer repo to the installed system for the first-boot flow.
///
/// Creates the installed system flake directory on the target and copies
/// the selected flake repo snapshot with a `.first-boot-pending` marker so the
/// TUI knows to run the first-boot wizard on next login.
async fn copy_config_to_target(
    ...
    let copy_status = Command::new("cp")
        ...
        .output()
        .await
        .map_err(|e| format!("Failed to start repo copy: {}", e))?;
    if !copy_status.status.success() {
        return Err(format!(
            "Failed to copy installer repo: {}",
