...
impl FirstBootScreen {
    pub fn new(config: FirstBootConfig) -> Self {
        ...
        let mut remote_input = TextInput::new()
            .with_placeholder("git@github.com:user/nixos-config.git");
    ...
    fn spawn_push(&mut self) {
        ...
        tokio::spawn(async move {
            ...
            let remote_exists = remote_check
        ...
        for msg in messages {
            match msg {
                ...
                FirstBootMessage::PushResult(Ok(())) => {
                    self.output_lines
                        .push("Push successful!".to_string());
                    self.finish();
                }
...
mod tests {
    ...
    fn test_poll_push_failure() {
        ...
        tx.send(FirstBootMessage::PushResult(Err(
