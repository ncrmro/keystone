...
**REQ-004.8** Every CLI subcommand MUST support a `--json` flag that
switches the command to structured JSON I/O mode. In this mode, input
is read from stdin as a JSON object and output is written to stdout as
a JSON object.
...
**REQ-004.11** The existing `--json` flag for config generation (stdin
JSON, outputs repo path) MUST continue to work as a compatibility alias
for the `template --json` subcommand.
