...
wired and WiFi connectivity, the Keystone TUI installer, and disko +
...
**REQ-008.13** The install flow MUST: show config summary, allow disk
selection, confirm before erasing, run disko for partitioning, and run
nixos-install with the embedded configuration.

**REQ-008.14** After successful install, the TUI MUST run
...
**REQ-008.15** The TUI MUST commit the hardware config changes to the
local git repository on the installed system. The TUI MUST NOT attempt
to push — the ISO does not contain private SSH keys.

**REQ-008.16** The TUI MUST write a `.first-boot-pending` marker to the
installed system's config directory so the first-boot flow triggers on
next login.

## Stage 5: First Boot (target machine, installed system)

**REQ-008.17** On first boot, the TUI MUST detect the `.first-boot-pending`
marker and enter the first-boot wizard.

**REQ-008.18** The first-boot wizard MUST guide the user through Secure
Boot key enrollment via `sbctl enroll-keys --microsoft`.

**REQ-008.19** The first-boot wizard MUST guide the user through TPM2
enrollment for automatic disk unlock.

**REQ-008.20** After Secure Boot and TPM enrollment, the TUI SHOULD
...
**REQ-008.21** The TUI MUST detect when Secure Boot and TPM are enrolled
and skip those steps on subsequent boots.

**REQ-008.22** The TUI MUST guide the user through SSH key setup. Options
...
**REQ-008.23** Once SSH keys are configured, the TUI MUST prompt the user
to push the pending hardware config commit (from Stage 4) to the remote
repository.

**REQ-008.24** The TUI SHOULD guide the user through initializing agenix
secrets for the system.

## Stage 7: Services + Onboarding (target machine)

**REQ-008.25** The TUI SHOULD present the Services screen showing which
`keystoneServices` are available and which host they are assigned to.

**REQ-008.26** The TUI SHOULD guide the user through creating secrets
required by enabled services.

**REQ-008.27** The TUI SHOULD present a brief onboarding tutorial or
next-steps summary after all setup is complete.

**REQ-008.28** After the full onboarding journey is complete, the TUI
