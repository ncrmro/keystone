...
resolve_keystone_docs_root() {
  ...
  if [[ -n "$current_repo" && -f "$current_repo/docs/ks.md" && -f "$current_repo/packages/ks-legacy/ks.sh" ]]; then
    printf '%s\n' "$current_repo"
    return 0
  fi

  local configured_repo="${HOME}/.keystone/repos/ncrmro/keystone"
  if [[ -f "$configured_repo/docs/ks.md" && -f "$configured_repo/packages/ks-legacy/ks.sh" ]]; then
    printf '%s\n' "$configured_repo"
    return 0
  fi
  ...
  if [[ -n "$repo_root" ]]; then
    if [[ -f "$repo_root/.repos/keystone/docs/ks.md" && -f "$repo_root/.repos/keystone/packages/ks-legacy/ks.sh" ]]; then
      printf '%s\n' "$repo_root/.repos/keystone"
      return 0
    fi

    if [[ -f "$repo_root/keystone/docs/ks.md" && -f "$repo_root/keystone/packages/ks-legacy/ks.sh" ]]; then
      printf '%s\n' "$repo_root/keystone"
      return 0
    fi
  fi
...
case "$CMD" in
  ...
  *)
    ...
    echo "Known commands: help, approve, build, update, agents, docs, sync-agent-assets, switch, sync-host-keys, grafana, print, agent, doctor" >&2
    exit 1
    ;;
esac
